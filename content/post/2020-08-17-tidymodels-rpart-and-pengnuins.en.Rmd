...
Exploring the data itself will be a separate blog post. So I am opting to remove all rows containing missing data. This will remove a total of 9 rows.
...
96% ROC AUC! Great results for a simple tree algorithm. I can visual the ROC AUC curves using the `roc_curve` function. Since this is a multiclass output and the ROC AUC is calculated as an average of the pairwise ROC AUC per class, there will have to be a plot per class.
