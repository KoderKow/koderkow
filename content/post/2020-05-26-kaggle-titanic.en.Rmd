...
tags:
  - tidymodels
  - kaggle
description: Exploration of the titanic data set on kaggle. Includes first run of using tidymodels!ß
featured_image: ~
---
...
all_data %>% 
  drop_na(survived) %>% 
  plot_two_variables(ticket_class, survived)
```
...
all_data %>% 
  drop_na(survived) %>% 
  plot_two_variables(sex, survived)
```
...
all_data %>% 
  drop_na(survived) %>% 
  plot_two_variables(age, survived)
```
...
all_data %>% 
  ...
  plot_two_variables(fare, survived)
```
...
all_data %>% 
  ...
  plot_two_variables(embarked_from, survived)
```
...
all_data %>% 
  ...
  plot_two_variables(family_size, survived)
```
...
all_data %>% 
  ...
  plot_two_variables(title, survived)
```
...
1. predict on all data (sub set columns)
2. add preds to all data
3. start plotting, where x is got it right or wrong
4. look for a signal